...
CREATE PROCEDURE SPREMOVE_WaveShop_ShoppingCart
(
    @idProductSelectedCart INT
)
AS
BEGIN
	IF EXISTS(SELECT 1 FROM [ProductSelectedCart] WHERE id = @idProductSelectedCart)
		BEGIN
			BEGIN TRY 

				BEGIN TRANSACTION;

					DELETE FROM [ProductSelectedCart] WHERE 
					(
						[ProductSelectedCart].[id] = @idProductSelectedCart
					)
